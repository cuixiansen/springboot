...
<body>
<!--  ======================== 目录搜索================== -->
     <shiro:hasPermission name="category:insert"> <ul class="nav navbar-nav">
     ...
     <shiro:hasPermission name="category:select">   <div class="navbar-form navbar-left" role="search">
...
function autoadd(value, row ,index){
	 return "<shiro:hasPermission name='category:delete'><a type='button' class='btn btn-info btn-sm' onclick=del('"+value+"')>删除 <span class='glyphicon glyphicon-remove  aria-hidden='true'></span></a></shiro:hasPermission>&nbsp;&nbsp;&nbsp;&nbsp;"+
	 "<shiro:hasPermission name='category:update'><a type='button' class='btn btn-info btn-sm' data-toggle='modal' data-target='#myModal' onclick=modify('"+value+"')>修改  <span class='glyphicon glyphicon-wrench ' aria-hidden='true'></span></a></shiro:hasPermission>"
}
//--------------------------删除方法----------------
 function del(id){
	 if(confirm("确定删除?")){
	...
	function update() {
		if (p1 && pn) {
			$.ajax({
				url : "updateCate",
				type : "post",
				data : $("#categoryform").serialize()+"&_method=PUT",
				success : function(msg) {
